...
public interface EmployeeRepository extends EmployeeRepositoryBase {
	...
	@Override
	//	@PreAuthorize("@employeeRepository.findOne(#id)?.createdByUser.userName == authentication?.name  ||  hasRole('admin')")         
	public Employee findOne(@Param("id") Long id);

	@Override
	//	@PreAuthorize("#employee?.createdByUser.userName == authentication?.name  ||  hasRole('admin')")         
